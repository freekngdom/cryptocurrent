...
var coins = ["btc", "eth", "bch", "ada", "neo", "xlm", "xmr", "eos", "miota", "dash", "trx", "ven", "qtum", "nano", "lsk", "omg", "icx", "zec", "bnb", "strat", "doge", "wtc", "zrx", "kmd", "ark", "bat", "kcs", "gas", "aion", "lrc", "elf", "eng", "req", "enj", "dbc", "xby", "ast", "evx", "viu", "xnn"];
...
var additionalData = ["All Time High", "Social Sentiment", "Market Cap USD", "Market Cap %", "My Coins", "Current BTC Value", "Current USD Value", "Percent", "Local Wallet Quantity", "Binance", "GDAX", "others", "Purchase Price USD", "Amount Paid USD", "Purchase Price BTC", "Amount Paid BTC", "Fee", "Fee Coin", "Fee %", "Fee USD", "$Gain", "%Gain", "25%", "10%", "-5%", "Local%", "Local Wallet Value", "Exchange %", "Exchange Value", "Public Addresses", "ERC20 Contract Address", "Description / Notes"];
...
function setCmcData(sheet) {
    var cmcRangeValues = sheet.getRange(headerRow + 1, initialColumn, coins.length, cmcData[0].length).getValues();

    for (var r in coins) {
        if (coins.hasOwnProperty(r)) {
            var row = r + header + 1;
            // check [ "id", "name", "symbol" ] for coin ... initialColumn...initialColumn+2
            var coin = sheet.getRange(row, initialColumn, 1, initialColumn + 2).filter(String)[0];
            for (var c in cmcData[0]) {
                if (cmcData[0].hasOwnProperty(c)) {
                    var column = initialColumn + c;
                    var template = sheet.getCell(row, column);
...
function SETUP() {
    function setRow(data, sheet, row, column) {
        var range = sheet.getRange(row, column, 1, data.length);
    ...
    function setColumn(data, sheet, row, column) {
        ...
        var range = sheet.getRange(row, column, data.length, 1);
