...
var coins = ["btc", "eth", "bch", "Cardano", "neo", "xlm", "xmr", "eos", "miota", "dash", "trx", "ven", "qtum", "nano", "lsk", "omg", "icx", "zec", "bnb"];
var exchanges = ["Binance", "GDAX", "Poloniex", "Bitrex", "Cryptopia", "Bitfinex", "Nanex"];
var additionalData = ["All Time High", "Social Sentiment", "24h Volume %", "Market Cap %", "Coins Owned", "Current USD Value", "Current BTC Value", "Portfolio Percentage", "Local Wallet Quantity", "Others", "Purchase Price USD", "Amount Paid USD", "Purchase Price BTC", "Amount Paid BTC", "Fee", "Fee Coin", "Fee %", "Fee USD", "$Gain", "%Gain", "25%", "10%", "-5%", "Public Addresses", "ERC20 Contract Address", "Description / Notes"];
// additionalData.splice( additionalData.indexOf( "Local Wallet Quantity" ), 0, exchanges );

var test = getSheetWithName("test");
var headerRow = 5;
var initialColumn = 1;
...
function coinValueFrom(coin, key) {
    coin = coin.trim().toLowerCase().replace(/ /g, "-");
    ...
    var filteredObj = obj.filter(function (thisObj) {
        return thisObj.id === coin || thisObj.symbol.toLowerCase() === coin;
    });
...
function SETUP() {
    additionalData.splice.apply(additionalData, [additionalData.indexOf("Local Wallet Quantity") + 1, 0].concat(exchanges));
    var headers = Object.keys(cmcData[0]).concat(additionalData);
...
function UPDATE() {
    setCmcData(test);
    var valueColumns = ["Current USD Value", "Current BTC Value"];
    valueColumns.forEach(function (column) {
        var currentCurrency = column.split(" ")[1].toLowerCase();
...
function setCmcData(sheet) {
    for (var i = 0; i < coins.length; i += 1) {
        ...
        var coin = sheet.getRange(row, initialColumn, 1, initialColumn + 2).getValues()[0].filter(String)[0];
        for (var j = 0; j < Object.keys(cmcData[0]).length; j += 1) {
            var template = Object.keys(cmcData[0])[j];
            var column = initialColumn + j;
...
function updateValues(currency, column, sheet) {
    var totalCell = sheet.getRange(headerRow - 1, column);
    var totalRange = sheet.getRange(headerRow + 1, column, coins.length, 1);
    var ownedColumn = getColumnWithName("Coins Owned", sheet);
    var localOwnedColumn = getColumnWithName("Local Wallet Quantity", sheet);
    var numberOwnedColumns = exchanges.length + 2;
    var currentPriceColumn = getColumnWithName("price_" + currency, sheet);
    // currentValue = currentPrice * AmountOwned;
    for (var i = 0; i < coins.length; i += 1) {
        var currentPrice = sheet.getRange(headerRow + 1 + i, currentPriceColumn).getValue();
        var owned = sheet.getRange(headerRow + 1 + i, localOwnedColumn, 1, numberOwnedColumns).getValues()[0].reduce(function (x, y) {
            return x + y;
        });
        var currentValue = currentPrice * owned;
...
function updatePercents(sheet) {
    var portfolioPercentageColumn = getColumnWithName("Portfolio Percentage", sheet);
    var totalVolume = sheet.getRange(2, getColumnWithName("24h_volume_usd", sheet)).getValue();
    var totalMarketCap = sheet.getRange(2, getColumnWithName("market_cap_usd", sheet)).getValue();
    for (var i = 0; i < coins.length; i += 1) {
        var currentCoinValue = sheet.getRange(headerRow + 1 + i, getColumnWithName("Current USD Value", sheet)).getValue();
        var TotalValue = sheet.getRange(headerRow - 1, getColumnWithName("Current USD Value", sheet)).getValue();
        var percent = currentCoinValue / TotalValue;
        sheet.getRange(headerRow + 1 + i, portfolioPercentageColumn).setValue([percent]);

        var coinVolume = sheet.getRange(headerRow + 1 + i, getColumnWithName("24h_volume_usd", sheet)).getValue();
        var volumePercent = coinVolume / totalVolume;
        ...
        var coinMarketCap = sheet.getRange(headerRow + 1 + i, getColumnWithName("market_cap_usd", sheet)).getValue();
        var marketCapPercent = coinMarketCap / totalMarketCap;
        sheet.getRange(headerRow + 1 + i, getColumnWithName("Market Cap %", sheet)).setValue([marketCapPercent]);
    }
    sheet.getRange(headerRow - 1, portfolioPercentageColumn).setValue(sum(sheet.getRange(headerRow + 1, portfolioPercentageColumn, coins.length, 1).getValues()));
    var priceColumn = getColumnWithName("price_usd", sheet);
    var wallets = ["Local Wallet Quantity", "Others"].concat(exchanges);
    var priceRange = sheet.getRange(headerRow + 1, priceColumn, coins.length, 1).getValues().reduce(function (x, y) {
        return x.concat(y);
    }, []);
    wallets.forEach(function (wallet) {
        var quantityColumn = getColumnWithName(wallet, sheet);
        var quantityRange = sheet.getRange(headerRow + 1, quantityColumn, coins.length, 1).getValues().reduce(function (x, y) {
            return x.concat(y);
        }, []);
        if (quantityRange.length > 0) {
            var valueCell = sheet.getRange(headerRow - 1, quantityColumn);
            var percentCell = sheet.getRange(headerRow - 2, quantityColumn);
            var balance = sumProduct(quantityRange, priceRange);
            var totalBalance = sheet.getRange(headerRow - 1, getColumnWithName("Current USD Value", sheet)).getValue();
...
function globalPercentChange(time, sheet) {
    var currentColumn = getColumnWithName("percent_change_" + time, sheet);
    var currentCell = sheet.getRange(headerRow - 2, currentColumn);
    var totalCell = sheet.getRange(headerRow - 1, currentColumn);
    var currentRange = sheet.getRange(headerRow + 1, currentColumn, coins.length, 1).getValues().reduce(function (x, y) {
        return x.concat(y);
    }, []);
    var portfolioPercentageRange = sheet.getRange(headerRow + 1, getColumnWithName("Portfolio Percentage", sheet), coins.length, 1).getValues().reduce(function (x, y) {
        return x.concat(y);
    }, []);
    var percent = sumProduct(currentRange, portfolioPercentageRange);
    var myTotal = sheet.getRange(headerRow - 1, getColumnWithName("Current USD Value", sheet)).getValue();
    currentCell.setValue([percent]);
    var total = percent * myTotal;
...
function getColumnWithName(columnName, sheet) {
    var headers = sheet.getRange(headerRow, initialColumn, 1, sheet.getLastColumn() - initialColumn).getValues()[0];
