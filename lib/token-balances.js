...
var ethAddress = "0x1d115b5c849f534b2c44cfa5078ca0de455895cc";
var apiKey = "freekey";
var apiServer = "https://api.ethplorer.io/getAddressInfo/";
var url = "" + apiServer + address + "?apiKey=" + apiKey;
var json = {
...
function ethBalance(ethAddresses) {
    ...
    var addresses = new Set(ethAddresses);
    var tokens = [];
    var ethBalance = 0;
    addresses.forEach(function (address) {
        ...
        tokens.push(json.tokens.forEach(function (token) {
            var tokenBalance = token.balance.toFixed() / ("1.0e" + token.tokenInfo.decimals).toFixed();
            return { name: token.tokenInfo.name, symbol: token.tokenInfo.symbol, balance: tokenBalance };
        }));
    });
    return { ethBalance: ethBalance, tokens: tokens };
}